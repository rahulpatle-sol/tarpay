...
export const useAuth = create((set) => ({
  ...
  login: async (upiId, password) => {
    set({ loading: true });
    try {
      const { data } = await authAPI.login({ upiId, password });
      await SecureStore.setItemAsync("tarpay_token", data.token);
  ...
  restore: async () => {
    try {
      ...
      set({ user: data.user, token: tok, ready: true });
      return true;
    } catch { set({ ready: true }); return false; }
  },

  refreshUser: async () => {
    try { const { data } = await userAPI.me(); set({ user: data.user }); } catch {}
  },

  setUser: (u) => set({ user: u }),

  logout: async () => {
    await SecureStore.deleteItemAsync("tarpay_token");
